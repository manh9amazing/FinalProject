...
public class Enemy2 extends GameObject {
    ...
    public Enemy2(){
        this.HP = 100;
        this.image = SpriteUtils.loadImage("assets/images/players/straight/enemy2.png");
        this.position = new Vector2D(400,360);
    ...
    public void castSpell() {
        for (int i = 0 ; i < spellNum  ; i++){
            if(i% 2 ==1 && this.dir_switch==true) {
                double tempAng = (180 / (spellNum - 1)) * i;
                double radConvert = Math.toRadians(tempAng);
                Enemy2Spell newSpell = GameObject.recycle(Enemy2Spell.class);
                newSpell.createAngle = radConvert;
                newSpell.position.x = this.position.x + 19;
                newSpell.position.y = this.position.y + 50;
            }
